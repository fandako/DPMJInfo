...
public class ConnectionAdapter extends BaseAdapter<BaseAdapter.BaseViewHolder, Connection> {
    ...
    public class ViewHolder extends BaseAdapter.BaseViewHolder {
        ...
        private LinearLayout getDepartureView(Context context, BusStopDeparture departure, boolean withLine) {
            int layout = R.layout.busstop_departure_list_item;
            boolean isFootConnection = (departure.getLineId() == ConnectionQuery.FOOT_CONNECTION_LINE_ID);

            //check for foot connections as it has different layout
            if(isFootConnection && withLine){
                layout = R.layout.connection_foot_list_item;
            }

            LinearLayout l = (LinearLayout) LayoutInflater.from(context).inflate(layout, null, false);
